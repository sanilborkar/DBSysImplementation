...
int add_data (FILE *src, int numrecs, int &res) {
	DBFile dbfile;
	//cout << "add_test: Opening .. " << rel->path ();
...
void RenameFile(){
	...
	char *new_fname = (char*) malloc(strlen(rel->path()) + 5);	// -4 (for SORT_FILE) + 1 ('\0') = -3
	sprintf(new_fname, "%s%s", rel->path(), ".out");	// 4 = SORT_FILE 
	if (rename(new_fname, rel->path()))
		cerr << endl << "Error renaming file from " << rel->path() << " to " << new_fname;

	char *old_meta = (char*) malloc(strlen(new_fname) + 6);
	sprintf(old_meta, "%s%s", new_fname, ".meta");

	char *new_meta = (char*) malloc(strlen(rel->path()) + 6);
	sprintf(new_meta, "%s%s", rel->path(), ".meta");	

	if (rename(old_meta, new_fname))
		cerr << endl << "Error renaming file from " << rel->path() << " to " << new_fname;
}